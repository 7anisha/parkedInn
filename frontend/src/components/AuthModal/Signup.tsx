...
const SignupForm = () => {
    ...
    const handleSignup = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true)
        try {
            const response = await AuthAPI.signup(email, mobile, password, name)
            localStorage.setItem('token', response.data.auth_token)
            API.defaults.headers.common['Authorization'] = `Bearer ${response.data.auth_token}`
            setUser(response.data.user)
            closeModal()
    ...
    return <>
        <form onSubmit={handleSignup}>
            <label className="label">
                <span className={`label-text ${errors.name ? "text-red-600" : ""}`}>Name {errors.name}</span>
            </label>
            <input
                type="text"
                name="fullName"
                placeholder="Enter your full name"
                className={`input input-bordered w-full mb-4 ${errors.name ? "input-error" : ""}`}
                onChange={() => setErrors((errors: ErrorType) => ({ ...errors, name: "" }))}
            />
            <label className="label">
                <span className={`label-text ${errors.email ? "text-red-600" : ""}`}>Email {errors.email}</span>
            </label>
            <input
                type="email"
                name="email"
            ...
            />
            <label className="label">
                <span className={`label-text ${errors.mobile ? "text-red-600" : ""}`}>Mobile {errors.mobile}</span>
            </label>
            <input
                type="mobile"
                name="mobile"
            ...
            <input
                ...
                onChange={() => setErrors((errors: any) => ({ ...errors, password: "" }))}
            />
        ...
        <div className="flex justify-between mt-4">
            <button className="btn btn-ghost hover:bg-transparent p-0 text-primary h-fit min-h-fit" onClick={() => setMode('login')}>Already have an account?</button>
        </div>
